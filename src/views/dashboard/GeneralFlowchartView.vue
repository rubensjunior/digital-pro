    ...
    <div class="nn-topbar">
      <div class="nn-topbar-left">
        ...
        <div class="nn-topbar-info">
          <span class="nn-topbar-title">Fluxograma Geral</span>
          <span class="nn-topbar-sub">
            {{ nosGrafo.length }} ideia{{ nosGrafo.length !== 1 ? 's' : '' }}
...
const LAYER_DIST = 120;     // espaço vertical entre camadas
const NODE_PAD_X = 40;      // espaço horizontal entre nós
...
function construirGrafo(correlacoes: IdeiaCorrelacao[]) {
  ...
  const mapList = new Map(todos.map(i => [i.id, i]));
  const roots = todos.filter(i => {
  ...
  for (const r of roots) {
    const w = getSize(r.id, new Set());
    posicionar(r.id, 0, null, null, currentX, currentX + w);
    currentX += w + NODE_PAD_X * 2;
  }

  const totalW = currentX - NODE_PAD_X * 2;
  const offsetX = -totalW / 2;
  const maxY = nos.reduce((max, n) => Math.max(max, n.y), 0);
  const offsetY = -maxY / 2;
  nos.forEach(n => { n.x += offsetX; n.y += offsetY; });
...
function onCanvasDblClick(e: MouseEvent) {
  ...
  if (no) {
    if (ideaDrawerRef.value) {
      ...
      console.warn('[GeneralFlowchart] Drawer ref não disponível no duplo clique');
    }
  }
}
...
function drawCanvas() {
  ...
  for (const no of nosGrafo.value) {
    ...
    ctx.lineWidth = isHov || isPainelNode ? 2.5 : 1.5;
    ctx.stroke();
